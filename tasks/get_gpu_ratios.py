...
import subprocess
import time
from utils import run_shell_command, CommandFailed, get_providers
import logging
import logs
...
def get_gpu_ratios(args):
    ...
    providers = get_providers(args["postcli_executable_dir"])
    ...
    if providers:
        for provider in range(providers):
            try:
                ...
                pass
            postcli_args = ['./postcli', '-provider', str(provider), '-commitmentAtxId', "00045b854f2baef1de75b753270e68b3b0dcee219167b84c6b7f302ca234ebe3", '-labelsPerUnit',
