...
def get_files_to_create_details(file_path):
    ...
    with open(file_path) as csv_f:
        ...
        for i, row in enumerate(csv_reader):
            files_to_create_details[int(i)] = {'hdd_path': row[0], 'num_units': int(row[1]), 'hdd_file_number': int(row[2])}
...
def delete_last_created_post_file(postdata_folder_path, min_max_provider_file_count: Optional[Tuple] = None):
    postdata_files = []
    ...
    for file in files:
        if "postdata_" in file and ".bin" in file:
            ...
            if min_max_provider_file_count:
                if postdata_number < min_max_provider_file_count(1) and postdata_number > (min_max_provider_file_count(0) - 1) :
...
def subprocess_values_as_string(subprocess_values: list):
    ...
    for gpu_value_arg in subprocess_values:
        values_as_str += str(gpu_value_arg) + ' '
...
def get_providers(postcli_executable_dir):
    return len([int(provider_id) for provider_id in re.findall(
        "ID: \(uint32\) (.*),",
        run_shell_command(f"{postcli_executable_dir}/./postcli -printProviders")) if provider_id != "4294967295"])


def create_postcli_file_threads(go_spacemesh_dir, postcli_executable_dir, postcli_file_hdd_path,
                                postcli_file_num_units, hdd_file_count: int, provider):
    ...
    if not "identity.key" in list_dir_files(postcli_file_path):
        ...
        return ['./postcli', '-provider', provider, '-commitmentAtxId', query_base_64_highest_atx, '-labelsPerUnit',
